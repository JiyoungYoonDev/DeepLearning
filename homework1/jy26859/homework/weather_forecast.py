...
class WeatherForecast:
    ...
    def find_the_largest_drop(self) -> torch.Tensor:
        ...
        """
        data_mean = torch.mean(self.data, dim=1)
        diff = torch.diff(data_mean)
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        data_mean = torch.mean(self.data, dim=1, keepdim=True)
        most = torch.argmax(torch.abs(self.data - data_mean), dim=1)
        rows = torch.arange(self.data.shape[0])

        return self.data[rows, most]
    ...
    def max_last_k_days(self, k: int) -> torch.Tensor:
        ...
        Returns:
            tensor of size (k,)
        """
        lask_k_days = self.data[-k:]
        max_temp = torch.max(lask_k_days, dim=1)
    ...
    def predict_temperature(self, k: int) -> torch.Tensor:
        ...
        """
        average = torch.mean(self.data[-k:])
    ...
    def what_day_is_this_from(self, t: torch.FloatTensor) -> torch.LongTensor:
        ...
        """
        days = torch.sum(torch.abs(self.data - t), dim= 1, keepdim=True)
        predicted = torch.argmin(days)
        return torch.Tensor.long(predicted)
